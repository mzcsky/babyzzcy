...
@interface UsermsgController ()<UITableViewDataSource,UITableViewDelegate>
...
@implementation UsermsgController
...
- (void)viewWillAppear:(BOOL)animated{
   ...
   // [_headIcon sd_setImageWithURL:[NSURL URLWithString:self.userBean.headImg]];
    [_headIcon sd_setImageWithURL:[NSURL URLWithString:self.userBean.headImg] placeholderImage:[UIImage imageNamed:@"ic_default_head_image"]];

    _nameLabel.text = self.userBean.realname;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    switch (indexPath.row) {
        ...
        case 0:
        {
            MySaiSaiController *ctrl = [[MySaiSaiController alloc] initWithUserId:[self.userBean.uId intValue] bOrAid:[self.userBean.uId intValue]];
        ...
        case 1:
        {
            MyAttendController *ctrller = [[MyAttendController alloc] initWithUserId:[self.userBean.uId intValue]];
        ...
        case 2:
        {
            MyFansController *ctrller = [[MyFansController alloc] initWithUserId:[self.userBean.uId intValue]];
            ctrller.m_showBackBt = YES;
            ctrller.title = @"粉丝";
            [self.navigationController pushViewController:ctrller animated:YES];
        ...
        case 3:
        {
            MPMController *ctrl = [[MPMController alloc] initWithUserId:[self.userBean.uId intValue]];
            ctrl.title = @"参与的比赛";
            ctrl.m_showBackBt = YES;
            [self.navigationController pushViewController:ctrl animated:YES];
            [[NSNotificationCenter defaultCenter] postNotificationName:HIDDEN_TAB object:nil];
